  ...
  Widget _pet() {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        // 仅保留点击弹跳；呼吸浮动 / 左右摇摆 / 身体跟随已移除
        final scale = 1.0 + bounceV * 0.12;
        final ty = -bounceV * 18;

        // 部件角度（顺序与 _parts 一致：尾、左耳、右耳、发带、左鱼鳍、右鱼鳍、蝴蝶结、呆毛、呆毛遮挡）
        final p = _kPart;
        final earA = p ? math.sin(_ear.value * 2 * math.pi) * 0.06 : 0.0;
        final earB = p ? math.sin(_ear.value * 2 * math.pi + 0.9) * 0.06 : 0.0;
        final ahogeA =
            p ? math.sin(_ahoge.value * 2 * math.pi) * 0.10 : 0.0;
        final tailA = p ? math.sin(_tail.value * 2 * math.pi) * 0.06 : 0.0;
        final finA = p ? math.sin(_fin.value * 2 * math.pi) * 0.07 : 0.0;
        final finB = p ? math.sin(_fin.value * 2 * math.pi + 1.1) * 0.07 : 0.0;
        final partAngles = [
          ...
          children: [
            ...
            Container(
              ...
              child: Text(
                ...
                '各部件围绕根部缓慢摆动，并会自然眨眼；点击桌宠可触发鼓励气泡。',
